...
<section class="border-t border-(--color-border) bg-(--color-surface-subtle)">
	<div class="mx-auto max-w-5xl px-6 py-8 sm:py-14">
		...
		<ul class="grid gap-6 sm:grid-cols-2">
			{#each [
				{ title: 'Technology strategy', body: 'Evaluate your current stack, surface hidden risks, and map out a realistic technical path aligned with your business goals and runway.' },
				{ title: 'Architecture reviews', body: 'A structured review of your system design, codebase, or technical plans with clear written recommendations you can act on.' },
				{ title: 'Hiring & team assessment', body: 'Help defining technical roles, writing job descriptions, and evaluating engineering candidates or agencies.' },
				{ title: 'Fundraising prep', body: 'I\'ve seen how VCs assess technical risk from inside a fund. I help you make your technical story credible before the room gets sceptical.' }
			] as item}
				<li class="rounded-xl border border-(--color-border) bg-white p-6">
...
<section class="border-t border-(--color-border) bg-(--color-surface-subtle)">
	<div class="mx-auto max-w-5xl px-6 py-8 sm:py-14">
		<div class="max-w-2xl">
			<h2 class="mb-6 text-2xl font-bold text-(--color-ink)">Background that's actually relevant</h2>
			<p class="mb-4 text-base leading-relaxed text-(--color-ink-muted)">
				I've written software for power plants where failure wasn't an option (Schneider Electric), bootstrapped a 20-person company from scratch, served as CTO at two VC-backed startups, and spent two and a half years as an Investment Associate at Cocoon Capital, one of Singapore's leading early-stage VC firms.
			</p>
			<p class="text-base leading-relaxed text-(--color-ink-muted)">
				I've also built on blockchain infrastructure, evaluated dozens of startups as an investor, and watched deals succeed and fail for reasons that had nothing to do with the product. That breadth is what I bring to advisory.
			</p>
		</div>
	</div>
</section>
